...
const AUDIENCE_OPTIONS = [
  { value: "all", label: "Everyone in State" },
...
export default function MinistryCommunicationsPage() {
  ...
  const { data: sentData, isLoading } = useQuery({
    queryKey: ["ministry", "broadcasts"],
    queryFn: () =>
      api
        .get<{ data: Broadcast[] }>("/ministry/broadcasts")
        .then((r) => r.data),
  });
  ...
  const { mutate: sendBroadcast, isPending } = useMutation({
    mutationFn: (payload: BroadcastForm) =>
      api.post("/ministry/broadcasts", payload).then((r) => r.data),
    onSuccess: () => {
      toast.success("Broadcast sent successfully");
      setForm({ title: "", body: "", audience: [], channels: [] });
      queryClient.invalidateQueries({ queryKey: ["ministry", "broadcasts"] });
    },
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Send messages to users in your state via multiple channels
          </p>
        </div>
      </div>
