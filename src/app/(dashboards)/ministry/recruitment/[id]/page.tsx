...
function statusBadge(status: string) {
  ...
  return (
    <Badge variant={map[status] ?? "gray"}>{status.replace(/_/g, " ")}</Badge>
  );
}
...
export default function RecruitmentDetailPage() {
  ...
  const [tab, setTab] = useState<
    "applications" | "import" | "interviews" | "ai_screening"
  >("applications");
  ...
  const [showBulkPersonalizedEmailModal, setShowBulkPersonalizedEmailModal] =
    useState(false);
  const [aiScreenResult, setAiScreenResult] = useState<{
    selected: number;
  ...
  const [bulkPersonalizedEmailForm, setBulkPersonalizedEmailForm] = useState({
    subject: "",
    message_template: "",
  });
  ...
  const campaign =
    (campaignRes as any)?.data?.data ?? (campaignRes as any)?.data;
  ...
  React.useEffect(() => {
    if (campaign) {
      setAiSettings({
        ...
        min_qualification:
          campaign.ai_screening_criteria?.min_qualification ?? "",
        min_experience_years:
          campaign.ai_screening_criteria?.min_experience_years?.toString() ??
          "",
        required_subject_keywords: (
          campaign.ai_screening_criteria?.required_subject_keywords ?? []
        ).join(", "),
        score_threshold:
          campaign.ai_screening_criteria?.score_threshold?.toString() ?? "70",
      });
    }
  ...
  const { data: appsRes, isLoading: appsLoading } = useQuery({
    ...
    queryFn: () =>
      get<any>(`/ministry/campaigns/${id}/applications?${appsParams}`),
  });
  const applications: Application[] =
    (appsRes as any)?.data?.data ?? (appsRes as any)?.data ?? [];
  ...
  const createInterview = useMutation({
    mutationFn: () =>
      post(`/ministry/campaigns/${id}/interviews`, interviewForm),
    onSuccess: () => {
  ...
  const aiScreen = useMutation({
    ...
    onSuccess: (res: any) => {
      ...
      qc.invalidateQueries({ queryKey: ["ministry-apps", id] });
      toast.success(
        `AI screening complete — ${result?.selected ?? 0} selected`,
      );
    },
  ...
  const updateCampaignAiSettings = useMutation({
    mutationFn: () =>
      patch(`/ministry/campaigns/${id}`, {
        ...
        ai_screening_criteria: {
          ...(aiSettings.min_qualification && {
            min_qualification: aiSettings.min_qualification,
          }),
          ...(aiSettings.min_experience_years && {
            min_experience_years: parseInt(aiSettings.min_experience_years),
          }),
          ...
          ...(aiSettings.score_threshold && {
            score_threshold: parseInt(aiSettings.score_threshold),
          }),
        },
      }),
    onSuccess: () => {
  ...
  const updatePosition = useMutation({
    mutationFn: ({ appId, position }: { appId: number; position: string }) =>
      patch(`/ministry/applications/${appId}/position`, {
        position_selected_for: position,
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["ministry-apps", id] });
  ...
  const toggleSelected = useMutation({
    mutationFn: ({ appId, value }: { appId: number; value: boolean }) =>
      patch(`/ministry/applications/${appId}/toggle-selected`, {
        is_ai_selected: value,
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["ministry-apps", id] });
    },
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
        ...
        <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
          ...
          <p className="font-semibold text-2xl text-blue-600">
            {
              applications.filter((a) => a.stage2_status === "shortlisted")
                .length
            }
          </p>
        </div>
      ...
      <div className="flex gap-1 border-b border-gray-200 dark:border-gray-700 flex-wrap">
        {(
          ["applications", "ai_screening", "import", "interviews"] as const
        ).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                >
                  ...
                  <option value="stage1_status:qualified">
                    Stage 1: Qualified
                  </option>
                  <option value="stage1_status:not_qualified">
                    Stage 1: Not Qualified
                  </option>
                  <option value="stage2_status:shortlisted">
                    Stage 2: Shortlisted
                  </option>
                  <option value="interview_status:attended">
                    Interview: Attended
                  </option>
                  <option value="final_status:recruited">
                    Final: Recruited
                  </option>
                </select>
                  ...
                  <tbody>
                    {applications.map((app) => (
                      ...
                      >
                        ...
                        <td className="py-3 pr-4">
                          {statusBadge(app.stage1_status)}
                        </td>
                        <td className="py-3 pr-4">
                          {statusBadge(app.stage2_status)}
                        </td>
                        <td className="py-3 pr-4">
                          {statusBadge(app.interview_status)}
                        </td>
                        <td className="py-3 pr-4">
                          {statusBadge(app.final_status)}
                        </td>
                        <td className="py-3">
                          <div className="flex gap-1">
                            ...
                            <Button
                              ...
                              onClick={() =>
                                toggleSelected.mutate({
                                  appId: app.id,
                                  value: !app.is_ai_selected,
                                })
                              }
                              disabled={toggleSelected.isPending}
                            >
                              <Sparkles
                                className={`h-4 w-4 ${app.is_ai_selected ? "text-amber-500" : "text-gray-400"}`}
                              />
                            </Button>
                          </div>
                        </td>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Export your Google Form responses as CSV and upload here. The
              following columns are mapped automatically:
            </p>
            <div className="overflow-x-auto">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-sm text-gray-500">
                          ...
                          {new Date(iv.ends_at).toLocaleString()}
                        </p>
                        <div className="flex gap-2 items-center text-sm">
                          <Badge
                            variant={iv.mode === "virtual" ? "blue" : "gray"}
                          >
                            {iv.mode}
                          </Badge>
                          {iv.venue && (
                            <span className="text-gray-500">{iv.venue}</span>
                          )}
                        </div>
                        {iv.notes && (
                          <p className="text-xs text-gray-400">{iv.notes}</p>
                        )}
                        {iv.livekit_room && (
                          <p className="text-xs text-brand font-mono">
                            Room: {iv.livekit_room}
                          </p>
                        )}
                      </div>
      ...
      {tab === "ai_screening" && (
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setAiSettings((f) => ({
                        ...f,
                        min_qualification: e.target.value,
                      }))
                    }
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAiSettings((f) => ({
                        ...f,
                        required_subject_keywords: e.target.value,
                      }))
                    }
              ...
              <div className="flex justify-end">
                <Button
                  onClick={() => updateCampaignAiSettings.mutate()}
                  disabled={updateCampaignAiSettings.isPending}
                >
                  {updateCampaignAiSettings.isPending
                    ? "Saving..."
                    : "Save AI Settings"}
                </Button>
              </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                The AI will score all pending Stage 1 applications based on the
                criteria above and automatically move qualified applicants to
                the Selected List.
              </p>
              <div className="flex flex-wrap items-center gap-3">
                <Button
                  ...
                  disabled={
                    aiScreen.isPending || !campaign?.ai_screening_enabled
                  }
                  className="gap-2"
                >
                ...
                {!campaign?.ai_screening_enabled && (
                  <p className="text-xs text-amber-600">
                    Enable AI Screening in settings above and save before
                    running.
                  </p>
                )}
              </div>
                ...
                {selected.size > 0 && (
                  <div className="flex gap-2 flex-wrap">
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => setShowBulkPersonalizedEmailModal(true)}
                    >
                ...
                if (selectedApps.length === 0) {
                  return (
                    <p className="text-sm text-gray-500 py-4">
                      No selected applicants yet. Run AI Screening above, or use
                      the{" "}
                      <Sparkles className="h-3.5 w-3.5 inline text-gray-400" />{" "}
                      button in the Applications tab to manually add applicants.
                    </p>
                  );
                }
                return (
                      ...
                      <thead>
                        <tr className="border-b border-gray-200 dark:border-gray-700 text-left">
                          <th className="py-2 pr-3">
                            ...
                            >
                              {selectedApps.every((a) => selected.has(a.id)) &&
                              ...
                              {editingPositionId === app.id ? (
                                <div className="flex gap-1 items-center">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setEditingPositionVal(e.target.value)
                                    }
                                    autoFocus
                                    onKeyDown={(e) => {
                                      if (e.key === "Enter")
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    setEditingPositionId(app.id);
                                    setEditingPositionVal(
                                      app.position_selected_for ?? "",
                                    );
                                  }}
                                >
                                  {app.position_selected_for ??
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setAddForm((f) => ({ ...f, applicant_name: e.target.value }))
                }
                required
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setAddForm((f) => ({ ...f, applicant_email: e.target.value }))
                }
                required
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setAddForm((f) => ({ ...f, applicant_phone: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setAddForm((f) => ({ ...f, subject_area: e.target.value }))
                }
              />
            </div>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button type="submit" disabled={addApp.isPending}>
              {addApp.isPending ? "Saving..." : "Add Application"}
            </Button>
          </div>
        </form>
      </Modal>
      ...
      <Modal
        ...
        title={
          singleApp
            ? `Update Stage — ${singleApp.applicant_name}`
            : `Bulk Update Stage (${selected.size} selected)`
        }
      >
        <div className="space-y-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setStageForm((f) => ({
                  ...
                  status:
                    STATUS_OPTIONS[e.target.value]?.[1]?.value ?? "pending",
                }))
              }
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setStageForm((f) => ({ ...f, status: e.target.value }))
              }
            >
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setStageForm((f) => ({ ...f, review_notes: e.target.value }))
              }
            />
          </div>
          <div className="flex justify-end gap-2">
            ...
            <Button
              onClick={() =>
                singleApp ? singleStage.mutate() : bulkStage.mutate()
              }
              disabled={singleStage.isPending || bulkStage.isPending}
            >
              {singleStage.isPending || bulkStage.isPending
                ? "Updating..."
                : "Update"}
            </Button>
          </div>
      ...
      <Modal
        ...
        title={`Send Email to ${selected.size} applicant(s)`}
      >
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            ...
            <select
              ...
              onChange={(e) =>
                setEmailForm((f) => ({
                  ...f,
                  template: e.target.value as EmailTemplate,
                }))
              }
            >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setEmailForm((f) => ({
                    ...f,
                    interview_schedule_id: e.target.value,
                  }))
                }
              >
                ...
                {interviews.map((iv) => (
                  <option key={iv.id} value={String(iv.id)}>
                    {iv.title} ({new Date(iv.starts_at).toLocaleDateString()})
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setInterviewForm((f) => ({ ...f, starts_at: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setInterviewForm((f) => ({ ...f, ends_at: e.target.value }))
                }
              />
            </div>
          </div>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              onClick={() => {
                if (
                  !interviewForm.title ||
                  !interviewForm.starts_at ||
                  !interviewForm.ends_at
                ) {
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            ...
            <code className="bg-gray-100 dark:bg-gray-800 px-1 rounded">{`{position}`}</code>{" "}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setBulkPersonalizedEmailForm((f) => ({
                  ...f,
          ...
          <div className="flex justify-end gap-2">
            <Button
              variant="ghost"
              onClick={() => setShowBulkPersonalizedEmailModal(false)}
            >
              Cancel
            </Button>
            <Button
              onClick={() => {
                if (
                  !bulkPersonalizedEmailForm.subject ||
                  !bulkPersonalizedEmailForm.message_template
                ) {
